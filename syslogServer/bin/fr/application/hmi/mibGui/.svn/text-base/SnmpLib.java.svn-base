...
public class SnmpLib implements ISnmpLib {
	...
	private Target getTarget(String strCommunity) {
		if (m_target == null) {
			Address addr = GenericAddress.parse("udp:" + getHost() + "/"
					+ getPort());
	...
	public void snmpWalk(String oidFrom) throws SnmpException {
		...
		try {
			...
			do {
				requests++;
				ResponseEvent responseEvent = snmp.send(request,
						getTarget(getReadCommunity()));
		...
		if (m_snmpResponseHandler != null) {
			m_snmpResponseHandler.requestStats(requests, objects,
	...
	void snmpSetValue(String oid, int syntax, String value)
			throws SnmpException {
