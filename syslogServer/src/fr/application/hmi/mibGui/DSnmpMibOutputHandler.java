...
public class DSnmpMibOutputHandler {
	...
	void showErrorMessage(String s) {
		JOptionPane.showMessageDialog(outputText, s, "Mib Browser",
	...
	public void setLogging(boolean log) {
		...
		try {
			if (log == true) {
				...
				outfile.write("\n**********************************************************\n");
				outfile.write("MIB Started at : " + new Date());
				outfile.write("\n**********************************************************\n");
				...
				System.out.println("Log will be refreshed every " + lFlushTime
						/ 1000 + " seconds.");
				...
				Thread thrdFlush = new Thread(new Runnable() {
					public void run() {
						try {
							outfile.write("\n**********************************************************\n");
							outfile.write("MIB Stopped at : " + new Date());
							outfile.write("\n**********************************************************\n");
							outfile.flush();
							outfile.close();
						} catch (Exception e) {
							System.out
									.println("Error while writing to log file: "
											+ e);
						}
					}
				});
	...
	private String getLogFileName() {
		...
		if (strFileName == null) {
			strFileName = Version.getNameTrim() + ".log";
		}
		return strFileName;
	}
	...
	private long getFlushTime() {
		...
		if (strTime != null) {
			try {
				...
				System.out
						.println("Invalid value for log refresh time. default will be used.");
			}
		}
